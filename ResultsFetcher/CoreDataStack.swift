...
class CoreDataManager {
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
        ...
        let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("ResultsFetcher.sqlite")
        var error: NSError? = nil
        var failureReason = "There was an error creating or loading the application's saved data."
        if let persistantStore = coordinator!.addPersistentStoreWithType(PersistanceStore.type, configuration: nil, URL: url, options: nil, error: &error) as? PersistanceStore {
...
extension CoreDataManager {
    func personInsertion() -> Person {
        return NSEntityDescription.insertNewObjectForEntityForName("Person", inManagedObjectContext: self.managedObjectContext!) as! Person
    }
    ...
    func testInsertion() {
        ...
        person.firstName = "Artemiy"
        person.secondName = "Sobolev"
        self.saveContext()
    }
}
